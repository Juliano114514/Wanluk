...
| 组词 | `phrases` | String? | 多个组词用英文逗号 `,` 分隔 | 否 |
...
字表为 **UTF-8 CSV**，首行为表头，**5497** 条数据行。表头共 17 列，落库只取其中 10 列语义。
...
| 呼 | `hu` | 原样 |
| 等 | `deng` | `一→1, 二→2, 三→3, 四→4`（**必须**在导入层转换后写入 `Int`） |
| 韻 | `yun` | 原样 |
| 調 | `diao` | 原样 |
| 組 | `zu` | 可空 |
| 攝 | `she` | 原样 |
| 單字 | `core_char` | 原样 |
| 组词 | `phrases` | 原样；UI 展示可用 `/` 分隔 |
| 多音 | — | **不入库**；见下方语义 |
| 原註 | `remark` | 非空则写入；与现场备注共用同一列 |
| 分韻(1782)…广州音白读 | — | **全部丢弃** |

**`多音` 列语义（0 / 1）**

- `多音=0`：常规范式，音韵坐标齐全。
- `多音=1` 且坐标齐全：同一 `單字` 的**独立音韵条目**（如「中」平上去各一条、「紅」不同义）。
- 坐标为空（约 **百条** 续行）：**继承上一条非空音韵行**的六维坐标，仍生成独立 `id`（例：「僮」书僮义与「僮族」义分两行）。
...
for each dataRow:
  ...
  entity = map(coords, 單字, 组词, 原註→remark, deng 中文→Int)
  buffer.add(entity)
insertWordCases(buffer) // 单事务
```

**解析注意**：历史注音列中偶发含逗号字段（如部分广州音列）；内置导入器应按**表头定列**或只读取前 11 列语义字段，避免按裸逗号拆列错位。用户自定义 CSV 一期要求字段内不含未转义逗号（引号转义放二期）。

**自定义 CSV 导入（用户字库）**

- 表头与上表「落库列」一致或为其子集；多余列忽略。
...
- 内置 CSV 须复制至 `app/src/main/assets/` 后方可灌库。
...
*文档版本：2026-06 · 字表行数以 `韵目表.csv`（5497 条数据行）为准。*
